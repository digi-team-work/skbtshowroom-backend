...
jQuery(function ($) {
    ...
    //region Box sides
    {
        ...
        const ownChangeEventMarker = { 'isAmeBoxSidesChange': true };
        //When the "link" option is enabled and the user changes the value of one input,
        //update all other inputs.
        const inputChangeEvents = 'change.ameBoxSidesChange input.ameBoxSidesChange';
        $('.ame-box-sides-control').each(function () {
            const $control = $(this);
            if ($control.data('ameIsComponent')) {
                return; //This is a Knockout component.
            }
            const $linkButton = $control.find('.ame-box-sides-link-button').first();
            const $numberInputs = $control.find('input.ame-box-sides-input');
