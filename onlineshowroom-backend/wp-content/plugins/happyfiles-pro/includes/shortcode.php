...
class Shortcode {
	...
	public static function render_block( $attributes = [], $content = '' ) {
		...
		$wrapper_classes = ['happyfiles-gallery'];
		...
		if ( $spacing !== false ) {
			echo "<style>#$gallery_id.happyfiles-gallery ul.wrapper { gap: $spacing; }</style>";
		}
		?>

		<figure id="<?php echo esc_attr( $gallery_id ); ?>" class="<?php echo implode( ' ', $wrapper_classes ); ?>">
			<ul class="wrapper" style="width: 100%;" data-col="<?php echo esc_attr( $columns ); ?>">
				<?php
				$attr = [];
